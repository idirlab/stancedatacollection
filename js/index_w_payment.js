...
function show_sentence(
    ...
    container.append(
        '<br><div id="top_well" class="well message highlight"><span id="span_REGION_status"></span></div><div class="panel panel-primary">  \
            ...
            <span id="span_tips"> \
                <h3>Tips for improving your pay rate!</h3>\
                <ul class="list-unstyled">\
                    ...
                    <li>(6) You may be tempted to pick easy/short claims to work on by clicking "Skip this pair". Keep in mind that our pay rate calculation formula has a component that accounts for the length/complexity of claims as well as how many pairs are skipped. We discourage excessive skipping. Nevertheless, if you are not confident about a question, it is still better to skip, because every single mistake will lower your pay rate.</li>\
                </ul>\
                \
                <br>Whenever you make one mistake, our algorithm lowers your pay rate, which means you get less points for every pair you have annotated. It takes multiple correct answers to make up for every single mistake and get the pay rate back to the previous value. If your current pay rate is 0 or very low, it is because our algorithm detected many mistakes in your answers. The best thing to do is to review your answers and modify them if necessary. If your pay rate is 0, it might actually be negative internally. If you continue to answer new questions, it will take MANY questions before you can see positive and improving pay rate.\
                <br><br>If you have only annotated 50-150 pairs, the pay rate based on the small sample may not reflect your true pay rate. It will become more robust once you have annotated more pairs.\
            </span>\
            <br><br><br>\
        </div>'
    );
    ...
    load_tweet_into_div(tweet_id);
}

function load_tweet_into_div(tweet_id) {
...
function update_payrate_message(
    ...
    if (is_training == 1) {
        ...
        $("#span_REGION_status").html(
            "Training Question: " +
                ...
                " (remaining " +
                remaining +
                ")<br>This is the training phase. Correct answer and explanation will be shown in this panel. The system only starts to calculate the leaderboard and pay rate after the training phase is over."
        );
    ...
    if (ANSWERED_message == null) {
        $("#button_leaderboard").prop("disabled", true);
        $("#span_REGION_status").html(
            "The <b>leaderboard</b> is enabled after your initial 50 pairs. After that, this message and the <b>leaderboard</b> are updated every 15-30 pairs. </b>See tips for improving your pay rate at the bottom of this page.</b>"
        );
    } else if (ANSWERED_message >= 50 && RANK_message != null) {
        ...
        $("#span_REGION_status").html(
            "Your estimated total payment is $" +
                ...
                " participants. Click 'Leaderboard' to see details. This message and the leaderboard are updated every 15-30 pairs. <b>See tips for improving your pay rate at the bottom of this page.</b>"
        );
    } else {
        $("#button_leaderboard").prop("disabled", true);
        $("#span_REGION_status").html(
